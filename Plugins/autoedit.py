...
async def editing(bot, message):
    if ("/set_cap" in message.text) and ((len(message.text.split(' ')) == 2) or (len(message.text.split(' ')) == 1)):
        ...
        await message.reply_text("⚠️\n\nA caption already seted for this channel id, you should first use /rmv_cap command to remove the current caption and then try to seting new.", quote=True)
    ...
    if ("/set_btn" in message.text) and ((len(message.text.split(' ')) == 2) or (len(message.text.split(' ')) == 1)):
        ...
        await message.reply_text("⚠️\n\nA button already seted for this channel id, you should first use /rmv_btn command to remove the current button and then try to seting new.", quote=True)
